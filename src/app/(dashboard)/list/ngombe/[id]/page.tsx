...
const NgombeDetails = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const result = await fetchNgombeData();
  ...
  return (
    <div className="bg-white p-6 rounded-md shadow-md m-4">
      ...
      <h1 className="text-xl font-semibold mb-4 text-lamaPurple">Ngombe Details</h1>
      <Table columns={columns} renderRow={renderRow} data={tableData} />

      <div className="mt-6">
        <Image src="/ngombe.png" alt="Ngombe" width={200} height={200} className="rounded-md" />
      </div>
    </div>
  );
};

export default NgombeDetails;