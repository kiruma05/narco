...
const RuvuRanchDetailsPage = () => {
    ...
    async function fetchData() {
      try {
        ...
        if (ngombeResult && ngombeResult.value && kondooResult && kondooResult.value) {
          ...
          setLoading(false);
          console.log("Fetched Data:", combinedData); // Debugging: Log fetched data
        } else {
          setLoading(false);
  ...
  const tableData = ranchData
    .filter((item) => {
      console.log(
        "Item Animal Data Code:",
        item.animal_data_one?.animal_code
      ); // Debugging: Log animal_data_one?.animal_code
      const isKondoo = typeof item.animal_data_one?.animal_code === "number";
      const isNgombe = !isKondoo;
  ...
  return (
    <div className="p-5 m-5">
      <h1 className="text-center text-cyan-950 font-semibold">RUVU Ranch Details</h1>
...
export default RuvuRanchDetailsPage;